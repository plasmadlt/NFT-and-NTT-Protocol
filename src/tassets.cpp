...
void tassets::updatever( string version )
{
  require_auth( get_self() );
  Configs configs( _self, _self.value );
  configs.set( tokenconfigs{ "tassets"_n, version }, _self );
}

void tassets::regauthor( name author, string data, string stemplate, string imgpriority )
{
  require_auth( author );
...
void tassets::authorupdate( name author, string data, string stemplate, string imgpriority )
{
  require_auth( author );
  require_recipient( author );
  authors author_( _self, _self.value );
...
void tassets::create( name author, name category, name owner, string idata, string mdata, bool requireclaim )
{
  require_auth( author );
...
void tassets::createlog( name author, name category, name owner, string idata, string mdata, uint64_t assetid, bool requireclaim )
{
  require_auth(get_self());
}

void tassets::claim( name claimer, std::vector<uint64_t>& assetids )
{
...
void tassets::transfer( name from, name to, std::vector<uint64_t>& assetids, string memo )
{
...
void tassets::update( name author, name owner, uint64_t assetid, string mdata )
{
  require_auth( author );
  ...
  assets_f.modify( itr, author, [&]( auto& a ) { a.mdata = mdata; });
}

void tassets::offer( name owner, name newowner, std::vector<uint64_t>& assetids, string memo )
{
...
void tassets::canceloffer( name owner, std::vector<uint64_t>& assetids )
{
  require_auth( owner );
  require_recipient( owner );
  offers offert( _self, _self.value );
...
void tassets::burn( name owner, std::vector<uint64_t>& assetids, string memo )
{
  require_auth( owner );
  sassets assets_f( _self, owner.value );
  offers offert( _self, _self.value );
...
void tassets::delegate( name owner, name to, std::vector<uint64_t>& assetids, uint64_t period, string memo )
{
...
void tassets::delegatemore( name owner, uint64_t assetidc, uint64_t period )
{
  require_auth( owner );
  require_recipient( owner );
...
void tassets::undelegate( name owner, name from, std::vector<uint64_t>& assetids )
{
  require_auth( owner );
  require_recipient( owner );
  ...
  for ( auto i = 0; i < assetids.size(); ++i )
  {
    ...
    check( itr->owner == itrc->delegatedto, "FROM does not match DELEGATED TO for at least one of the assets." );
...
void tassets::attach( name owner, uint64_t assetidc, std::vector<uint64_t>& assetids )
{
  sassets assets_f( _self, owner.value );
  delegates delegatet( _self, _self.value );
  offers offert( _self, _self.value );
...
void tassets::detach( name owner, uint64_t assetidc, std::vector<uint64_t>& assetids )
{
  require_auth( owner );
  require_recipient( owner );
  sassets assets_f( _self, owner.value );
...
void tassets::attachf( name owner, name author, asset quantity, uint64_t assetidc )
{
  attachdeatch( owner, author, quantity, assetidc, true );
}

void tassets::detachf( name owner, name author, asset quantity, uint64_t assetidc )
{
  attachdeatch( owner, author, quantity, assetidc, false );
}

void tassets::createf( name author, asset maximum_supply, bool authorctrl, string data )
{
  require_auth( author );
...
void tassets::updatef( name author, symbol sym, string data )
{
  require_auth( author );
  check( sym.is_valid(), "invalid symbol name" );
  stats statstable( _self, author.value );
  ...
  statstable.modify( existing, author, [&]( auto& a ) { a.data = data; });
}

void tassets::issuef( name to, name author, asset quantity, string memo )
{
  ...
  statstable.modify( *existing, same_payer, [&]( auto& s ) { s.supply += quantity; });
...
void tassets::transferf( name from, name to, name author, asset quantity, string memo )
{
...
void tassets::offerf( name owner, name newowner, name author, asset quantity, string memo )
{
  require_auth( owner );
  require_recipient( owner );
  require_recipient( newowner );
...
void tassets::cancelofferf( name owner, std::vector<uint64_t>& ftofferids )
{
  require_auth( owner );
  require_recipient( owner );
  offerfs offert( _self, _self.value );
...
void tassets::claimf( name claimer, std::vector<uint64_t>& ftofferids )
{
  require_auth( claimer );
  require_recipient( claimer );
  offerfs offert( _self, _self.value );
...
void tassets::burnf( name from, name author, asset quantity, string memo )
{
...
void tassets::openf( name owner, name author, const symbol& symbol, name ram_payer )
{
...
void tassets::closef( name owner, name author, const symbol& symbol )
{
  require_auth( owner );
  accounts acnts( _self, owner.value );
...
uint64_t tassets::getid( bool defer )
{
...
uint64_t tassets::getFTIndex( name author, symbol symbol )
{
  stats statstable( _self, author.value );
...
void tassets::attachdeatch( name owner, name author, asset quantity, uint64_t assetidc, bool attach )
{
  sassets assets_f( _self, owner.value );
  delegates delegatet( _self, _self.value );
  offers offert( _self, _self.value );
...
void tassets::sub_balancef( name owner, name author, asset value )
{
  ...
  from_acnts.modify( from, has_auth( author ) ? author : owner, [&]( auto& a ) { a.balance -= value; });
}

void tassets::add_balancef( name owner, name author, asset value, name ram_payer )
{
...
void tassets::createntt( name author, name category, name owner, string idata, string mdata, bool requireclaim )
{
  require_auth( author );
  check( is_account( owner ), "owner account does not exist" );
  require_recipient( owner );
...
void tassets::createnttlog( name author, name category, name owner, string idata, string mdata, uint64_t assetid, bool requireclaim )
{
  require_auth(get_self());
}

void tassets::updatentt( name author, name owner, uint64_t assetid, string mdata )
{
  require_auth(author);
...
void tassets::claimntt( name claimer, std::vector<uint64_t>& assetids )
{
  require_auth( claimer );
  require_recipient( claimer );
...
void tassets::burnntt( name owner, std::vector<uint64_t>& assetids, string memo )
{
  require_auth( owner );
  require_recipient( owner );
...
void tassets::sendEvent( name author, name rampayer, name seaction, const std::tuple<Args...> &adata )
{
  transaction sevent{};
...
asset tassets::get_supply( name token_contract_account, name author, symbol_code sym_code )
{
  stats statstable( token_contract_account, author.value );
  return statstable.get( sym_code.raw() ).supply;
}

asset tassets::get_balance( name token_contract_account, name owner, name author, symbol_code sym_code )
{
  stats statstable( token_contract_account, author.value );
